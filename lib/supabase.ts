...
export async function getFeaturedAds(locale = 'en', limit = 4): Promise<QueryResult<Ad>> {
  const { data, error } = await supabase
    .from('ads')
    .select<
      'id, title, description, price, currency, created_at, categories(id, slug, name_en, name_el, name_ru), locations(id, slug, name_en, name_el, name_ru), users(id, first_name, last_name)',
      AdWithRelations
    >('id, title, description, price, currency, created_at, categories(id, slug, name_en, name_el, name_ru), locations(id, slug, name_en, name_el, name_ru), users(id, first_name, last_name)')
    .eq('status', 'published')
    .eq('is_featured', true)
...
export async function getAdsByCategory(
  ...
  locale = 'en',
  limit = 10
): Promise<QueryResult<Ad>> {
  const { data, error } = await supabase
    .from('ads')
    .select<
      'id, title, description, price, currency, created_at, categories!inner(id, slug, name_en, name_el, name_ru), locations(id, slug, name_en, name_el, name_ru), users(id, first_name, last_name)',
      AdWithRelations
    >('id, title, description, price, currency, created_at, categories!inner(id, slug, name_en, name_el, name_ru), locations(id, slug, name_en, name_el, name_ru), users(id, first_name, last_name)')
    .eq('status', 'published')
